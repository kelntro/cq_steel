...
class Master extends DBConnection
{
	...
	function save_brand()
	{
		...
		$check = $this->conn->query("SELECT * FROM `type` where `name` = '{$name}' " . (!empty($id) ? " and id != {$id} " : "") . " ")->num_rows;
		if ($this->capture_err())
			return $this->capture_err();
		if ($check > 0) {
			$resp['status'] = 'failed';
			$resp['msg'] = "Roof Type Name already exist.";
			return json_encode($resp);
			exit;
		}
		if (empty($id)) {
			$sql = "INSERT INTO `type` set {$data} ";
		} else {
			$sql = "UPDATE `type` set {$data} where id = '{$id}' ";
		}
		...
		if ($save) {
			...
			$resp['status'] = 'success';
			if (empty($id))
				$resp['msg'] = "New Type successfully saved.";
			else
				$resp['msg'] = "Roof Type successfully updated.";
			if (!empty($_FILES['img']['tmp_name'])) {
				...
				elseif ($_FILES['img']['type'] == 'image/png')
					...
					$upload = false;
				if ($upload) {
					$qry = $this->conn->query("UPDATE type set `image_path` = CONCAT('{$fname}', '?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '{$bid}' ");
				}
				imagedestroy($temp);
			}
		} else {
	...
	{
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `type` where id = '{$id}'");
		if ($del) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', " Roof Type successfully deleted.");
		} else {
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function place_order()
	{
		...
		if ($save_order) {
			...
			$data = [];
			$cart = $this->conn->query("SELECT c.* from `cart` c where c.client_id ='{$client_id}' ");
			...
			$list_sql = "INSERT INTO `order_list` (order_id, inventory_id, quantity, price, total) VALUES " . implode(', ', $data);
			...
			if ($save_olist) {
				...
				$resp['status'] = 'failed';
				$resp['err_sql'] = $this->conn->error; // Store the error message
			}
		} else {
			$resp['status'] = 'failed';
			$resp['err_sql'] = $this->conn->error; // Store the error message
